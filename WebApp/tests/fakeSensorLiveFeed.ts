...
export default function(fetchedSensorData: Ref<SingleSensorReadingsType[]>) {

    fetchedSensorData.value.forEach((el)=>{
        ...
        el.id = el.id.concat(['FAKE'])
        el.temp = el.temp.concat([temp])
        el.rehu = el.rehu.concat([rehu])
        el.co2c = el.co2c.concat([co2c])
        el.time = el.time.concat([dateNow])
        

        console.log("R   Temp    Rehu    Co2C")
        console.log(el.room, el.temp.at(-1), el.rehu.at(-1), el.co2c.at(-1))
    })

    console.log("Fake data pushed")
